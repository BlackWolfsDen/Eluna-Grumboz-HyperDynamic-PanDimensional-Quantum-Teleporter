...
Teleporter = {
			{"|cff000000Eastern Kingdoms|r", 2, 2, 
					{
			...
			{"|cff000000Kalimdor|r", 2, 2, 
				...
				{"Durotar", 2, 1, 1, 1, -601.652100, -4238.582520, 38.956200, 4.618780},
			...
			{"|cff000000Classic Dungeons|r", 9, 2, 
					{
			...
			{"|cff000000Burning Crusades Dungeons|r", 9, 2, 
					{
			...
			{"|cff000000Custom Location|r", 10, 2, 
					{
			...
			{"|cff000000Event areas|r", 10, 2, {};
			},
					
			{"|cff000000GM Locations|r", 10, 3, {};
			},
...
local function TeleporterOnGossipSelect(event, player, unit, sender, intid, code)
	...
	if (intid > (intoffset))then
		...
		local name, icon, team, level, map, x, y, z, o = table.unpack(Teleporter[sender][4][int]);
	...
	if (sender >= 1)then-- Show teleport sub-menu
	    ...
	    local tSize = #Teleporter[sender][4];
	    local i = intid;
	    local entry = 0;
	    
		print("NAME: "..Teleporter[sender][1]);
		print("SIZE: "..tSize);
		print("P_TEAM: "..pTeam);
...
--  {"location name", icon, team, minimum level, map, x, y, z, o},}
			...
			local tTeam = Teleporter[sender][4][i][3];
			local tLevel = Teleporter[sender][4][i][4];

			print("T_TEAM: "..tTeam);
			print("T_LEVEL: "..tLevel);
				
	    	if((tTeam == 2) or (tTeam == pTeam) or player:IsGM())then
	    	
	    		if(tLevel <= pLevel)then
	
					print(":NAME: "..Teleporter[sender][4][i][1]);
					print(":ICON: "..Teleporter[sender][4][i][2]);
					print(":TEAM: "..Teleporter[sender][4][i][3]);
					print(":LEVEL: "..Teleporter[sender][4][i][4]);
					print("ADDED");
					print("");
					
	    			player:GossipMenuAddItem(Teleporter[sender][4][i][2], Teleporter[sender][4][i][1], sender, i+intoffset)
	   		...
	   		if(entry == 12)then
				player:GossipMenuAddItem(7, "next..", sender, i)
			...
			if(intid >= 12) then 
				player:GossipMenuAddItem(7, "Back..", 1, (intid - 12))
			end
